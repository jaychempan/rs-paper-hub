#!/usr/bin/env python3
"""
Filter Agent-related papers from cleaned data.

Usage:
    python filter_agent.py                          # Default: read papers.json
    python filter_agent.py --input output/x.json    # Custom input
    python filter_agent.py --dry-run                 # Preview without saving
"""
...
def main():
    parser = argparse.ArgumentParser(description="Filter Agent-related papers")
    parser.add_argument(
    ...
    # Filter
    matched, annotated = filter_agent_papers(papers)
    logger.info(f"Agent-related papers: {len(matched)} / {len(papers)}")

    # Show keyword distribution
    from collections import Counter
    keyword_counter = Counter()
    for p in matched:
        for kw in p["_agent_keywords"].split("; "):
            if kw:
    ...
    if args.dry_run:
        ...
        for p in matched[:10]:
            ...
            logger.info(f"       keywords: {p['_agent_keywords']}")
        return
    ...
    agent_papers = [{k: v for k, v in p.items() if k in columns} for p in matched]

    csv_path = os.path.join(args.output_dir, "papers_agent.csv")
    df = pd.DataFrame(agent_papers, columns=columns).fillna("")
    df.to_csv(csv_path, index=False, encoding="utf-8-sig")
    logger.info(f"Saved {len(agent_papers)} Agent papers to {csv_path}")

    json_path = os.path.join(args.output_dir, "papers_agent.json")
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(agent_papers, f, ensure_ascii=False, indent=2)
    logger.info(f"Saved to {json_path}")

    # Also save annotated full list
    annotated_path = os.path.join(args.output_dir, "papers_agent_annotated.json")
